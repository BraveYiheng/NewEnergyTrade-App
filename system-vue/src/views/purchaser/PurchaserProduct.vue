<template>
  <div class="purchaser-product-container">
        ...
        <el-table :data="searchResults" style="width: 100%" border stripe>
          ...
          <el-table-column label="操作" fixed="right" width="220">
            <template #default="scope">
              ...
              <el-button type="primary"  @click="openOrderDialog(scope.row)">下单</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
    ...
    <!-- 下单对话框 -->
    <el-dialog v-model="orderDialogVisible" title="确认下单" width="450px">
      <el-form :model="orderForm" label-width="80px">
        ...
        <el-form-item label="数量">
          <el-input-number v-model="orderForm.quantity" :min="1" :max="orderForm.stockQuantity"></el-input-number>
        </el-form-item>
        <el-form-item label="总价">
          <el-input :value="totalOrderAmount" disabled>
            <template #append>元</template>
          </el-input>
        </el-form-item>
      </el-form>
...
const totalOrderAmount = computed(() => {
    if(orderForm.unitPrice && orderForm.quantity) {
        return (orderForm.unitPrice * orderForm.quantity).toFixed(2);
    }
    return 0;
});
...
const openOrderDialog = (product) => {
  orderForm.productId = product.productId;
  orderForm.productName = product.productName;
  orderForm.quantity = 1;
  orderForm.unitPrice = product.price;
  orderForm.stockQuantity = product.stockQuantity;
  orderForm.supplierId = product.supplierId;
...
const submitOrder = async () => {
  ...
  const orderData = {
    ...
    orderAmount: totalOrderAmount.value,
    orderStatus: 0 // 默认订单状态为 0:待付款
  };
  try {
    await proxy.$http.post('/order/add', orderData);
